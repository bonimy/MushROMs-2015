...
namespace MushROMs.LunarCompress
{
    partial class LC
    {
        ...
        /// The <see cref="GraphicsFormat"/> value of the pixel data.
        /// </param>
        /// <returns>
        /// An <see cref="Int32"/> of the number of bits each pixel uses.
        /// </returns>
        public static int BitsPerPixel(GraphicsFormat graphicsFormat)
        {
        ...
        /// </returns>
        public static int BytesPerPlane(GraphicsFormat graphicsFormat)
        {
        ...
        /// </returns>
        public static int BytesPerTile(GraphicsFormat graphicsFormat)
        {
        ...
        /// for each pixel.
        /// </returns>
        public static int ColorsPerPixel(GraphicsFormat graphicsFormat)
        {
        ...
        /// </remarks>
        public static byte[] CreatePixelMap(byte[] source, int address, int numTiles, GraphicsFormat graphicsFormat)
        {
            if (source == null)
                return null;
        ...
        /// </remarks>
        public static bool CreatePixelMap(IntPtr source, IntPtr destination, int numTiles, GraphicsFormat graphicsFormat)
        {
            switch (graphicsFormat)
            {
                case GraphicsFormat.Mode7_8BPP:
                    unsafe
                    {
                        byte* src = (byte*)source;
                        byte* dest = (byte*)destination;
                        for (int i = numTiles * BytesPerTile(graphicsFormat); --i >= 0; )
                            dest[i] = src[i];
                    }
                    return true;
                default:
                    return UnsafeNativeMethods.LunarCreatePixelMap(source, destination, numTiles, graphicsFormat);
            }
        }

        /// <summary>
        /// Converts an indexed pixel map (such as one created with
        /// <see cref="CreatePixelMap(byte[], int, int, GraphicsFormat)"/>) into standard 8x8 SNES BPP Tiles.
        /// </summary>
        /// <param name="source">
        ...
        /// </remarks>
        public static byte[] CreateBPPMap(byte[] source, int address, int numTiles, GraphicsFormat graphicsFormat)
        {
            if (source == null)
                return null;
            ...
            unsafe
            {
                fixed (byte* src = &source[address])
                fixed (byte* dest = data)
                    return UnsafeNativeMethods.LunarCreateBppMap((IntPtr)src, (IntPtr)dest, numTiles, graphicsFormat) ? data : null;
            }
        }

        /// <summary>
        /// Converts an indexed pixel map (such as one created with
        /// <see cref="CreatePixelMap(IntPtr, IntPtr, int, GraphicsFormat)"/> into standard 8x8 SNES BPP Tiles.
        /// </summary>
        /// <param name="source">
        ...
        /// The <paramref name="source"/> and <paramref name="destination"/> variables must NOT point to the same array.
        /// </remarks>
        public static bool CreateBPPMap(IntPtr source, IntPtr destination, int numTiles, GraphicsFormat graphicsFormat)
        {
            switch (graphicsFormat)
            {
                case GraphicsFormat.Mode7_8BPP:
                    ...
                    return true;
                default:
                    return UnsafeNativeMethods.LunarCreateBppMap(source, destination, numTiles, graphicsFormat);
            }
        }
        ...
        {
            return UnsafeNativeMethods.LunarRender8x8(scan0, width, height, x, y, pixelMap, palette, Tile, flags);
        }
        #endregion
    }
}
